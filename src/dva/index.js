...
function dva(opts = {}) {
  const onError = opts.onError || function (err) {
    //wzy 该默认的onError的throw 没有捕捉，能够使程序崩溃，在这里，最好是不在抛出；
    //或者在启动dva，传入自己定义的onError方法；
    //console.debug(err)
    throw new Error(err);
  };
  ...
  function model(model) {
    ...
    check(model.namespace, namespace => namespace !== 'router', 'Namespace should not be router.');
    _models.push(model);
  }
  ...
  function start(container, opts = {}) {
    ...
    opts.sagas.forEach(saga => sagaMiddleware.run(saga));
